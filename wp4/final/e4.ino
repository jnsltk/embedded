...
void sort(int number, int tab[]) { // function to sort the array
    ...
    for (int i = 0; i < number; i++) { // iterate through the array
        for (int j = i; j < number - 1; j++) { // iterate from the current element to the end of the array
            if (tab[j] > tab[j + 1]) { // if the current element is greater than the next one
                temp = tab[j]; // assign the value of the current element to the temporary variable
                tab[j] = tab[j + 1]; // assign the value of the next element to the current element
                tab[j + 1] = temp; // assign the value of the temporary variable to the next element
            }
        }
    }
}
